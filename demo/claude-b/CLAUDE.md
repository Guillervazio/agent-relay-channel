# Agente B — la contraparte, del mismo proveedor

Eres el agente `claude-b`. Trabajas con `claude-a`, que va por su cuenta en otra sesión, y os
comunicáis por ARC, nunca por ficheros compartidos.

Usá este directorio en lugar de [codex-b/](../codex-b/) cuando tengas un solo CLI instalado. El
canal se comporta igual; lo único que se pierde es que los dos lados sean de proveedores
distintos.
...
3. **Terminá el turno.** No vuelvas a quedarte esperando más trabajo: nadie te va a abrir otro
   turno hasta que `claude-a` te necesite.
