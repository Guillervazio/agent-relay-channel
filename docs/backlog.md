...
| **`PRAGMA synchronous = NORMAL` is not in force.** It is emitted once, on the startup connection. The pragma is per connection and is not stored in the file, unlike `journal_mode` | Every connection the pool opens beyond the initialised handle runs at the `FULL` default, so the durability trade [persistence.project.md](../.claude/rules/persistence.project.md) calls a decision varies between operations. Not a correctness fault — `FULL` is safer, and slower than intended. **Due when:** now, by moving the pragma into `OpenAsync`. It is one line, and it is only here rather than done because the increment that found it was scoped to documentation |
| **A waiter can be orphaned and never woken.** `WaiterRegistry.Unregister` evicts a key's slot dictionary when it empties; a `Register` racing between the emptiness check and the eviction lands in a dictionary `Signal` no longer reaches | The waiter waits its full deadline. `AskAsync` and `AwaitResponseAsync` recover on the belt-and-braces re-read, but only after the timeout, which can be five minutes; `InboxAsync` re-reads only if it wakes, so the message is not seen in that call at all. It is the class of fault `WaiterRegistryTests` says it exists to catch, and no test covers it. **Due when:** now, on merit — the window is narrow but it opens on the ordinary polling pattern. The fix is to retry the registration when the dictionary it got has been evicted |
...
| **`ARC_MAX_WAIT` is not validated.** A negative value makes `ValidateWait` refuse every wait including `0`, so every channel request answers 422; an unparseable value falls back to 300 without a word | The hub starts, reports itself healthy, and does nothing. **Due when:** now, by refusing a non-positive value at startup the way an absent `ARC_TOKEN` already is |
