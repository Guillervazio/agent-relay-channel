...
| **`docs/arc-dev-environment.md` decays and nothing here notices.** Most of it is facts about one Windows host and about Codex CLI **v0.153.4** — flag names, a configuration key, what `exec` mode reports at startup, what the sandbox refuses. Both are stamped at the top of the page and neither is verifiable from this repository | It is the same shape as a pinned package with no instrument: a claim that goes quietly false, and the reader who is wrong is the one who trusted it. The page's own defence is that it says what it is true of, which puts the check on the reader rather than removing it. Note that the version stamp is the part that must **not** be edited to look current — that would remove the only thing making the staleness visible. **Due when:** somebody follows it and a command does not work, or that CLI is upgraded on this machine. Then re-check and re-stamp, or mark the section unverified |
| **The handshake text and `docs/AGENTS.md` are two copies and nothing keeps them in step.** [P014](adr/P014-the-channel-explains-itself-in-the-handshake.md) sanctioned the second copy for the two cases the handshake does not reach, and increment 14 added a clause to both **by hand** | The next person adds one to whichever they happen to have open, and the two say different things about the same channel — which is the drift P014 exists to prevent, reappearing inside the exception it granted. A mechanical check is not obvious and that is the honest part: one copy is Spanish prose for a model and the other is English prose for a person, so nothing compares them by string. What could be compared is the **topics**, the way `HubEndpointTests` already does for one side. **Due when:** a third clause is added to either, or somebody writes that comparison |
| **Nobody has opened a turn yet.** [P024](adr/P024-who-supplies-the-turn.md) settles who opens one and the demo is written for it, but no session has executed the arrangement: it needs a second CLI installed and a real turn command, and this increment deliberately produced neither | What is verified is the shape and the documents, which is the same standing the service installation has had since increment 06. What is unverified is everything the turn command touches: whether a one-turn invocation returns when the turn ends rather than hanging, whether the counterpart's thirty-second mailbox wait is margin enough, and what a turn command that fails looks like from the lead's side — today it is indistinguishable from a counterpart that is simply slow. **Due when:** somebody runs the demo, or a real session opens a turn for the first time.<br>**And it has grown past the turn.** Increments 13 to 16 shipped a demo nobody has run, a field report nothing here can re-check, and a template nobody has filled in. Each is defensible alone and together they are one thing: **the whole adoption story is written and unexercised.** The first real project to adopt ARC settles all of it at once, and what it finds should come back here rather than being fixed quietly in place |
