...
| **A base clause cannot be made to load for the file it governs.** `shared/build-and-packages.md` says a version declared there may be pinned in step with a container stage or a CI image; the `Dockerfile` and `.github/workflows/**` were added to the appendix's `paths:` by [P021](adr/P021-a-rule-that-never-arrived.md), and cannot be added to the base's, which is edited in `dotnet-house` | Opening the `Dockerfile` loads the appendix and not the base that carries the clause. The appendix's first line links to it, so the route exists and costs a second step. **Due when:** it is reported to the package along with the two entries above — the fix is a commit there. Do not edit the base's frontmatter from inside this repository; a base that differs from the package's copy is the drift the split exists to prevent |
