...
| **`AddResponseAsync` has an [H007](adr/house/H007-optimistic-concurrency-where-an-update-derives-from-a-read.md) hole.** `RespondAsync` checks `Status != Answered` in C#, then `UPDATE … WHERE id = $id AND kind = 'request'` runs with no condition on status | Two concurrent responders both pass the check and both insert a response row. The fix is one clause in the `WHERE` plus a rows-affected check. **Due when:** now — it is the first item of the next increment |
| **The error codes exist in four copies and no test asserts any of them.** `ChannelService`, `Arc.Hub/Program.cs`, `ArcTools`, and the table in `docs/PROTOCOL.md` | [H012](adr/house/H012-an-error-code-is-defined-once-and-a-test-keeps-the-literal.md) violated twice over. The 21 tests never touch `ChannelService`, so nothing would notice a code changing. **Due when:** with the H007 fix, since both land in `ChannelService` |
| **Four refusals answer 400 where [H013](adr/house/H013-422-for-a-well-formed-request-that-fails-validation.md) says 422**: `self_addressed`, `empty_body`, `body_too_large`, `bad_recipient` | Target and current state are both in the table in [api-guidelines.project.md](../.claude/rules/api-guidelines.project.md). `bad_agent` on the header is undecided. **Due when:** with the error-code consolidation, and `docs/PROTOCOL.md` in the same commit |
| **`Clamp` truncates an over-long `wait` silently.** `Math.Clamp(requested ?? 0, 0, MaxWaitSeconds)` | A caller asking 600 against a maximum of 300 gets 300, and its poll returns with an `outcome` indistinguishable from an ordinary timeout. **Due when:** with the 422 move above; it is the same shape of decision |
